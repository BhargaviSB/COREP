  ...
  addDownloadButtonListener: function (k) {
    ...
    downloadButton.addEventListener('click', (event) => {
          var uri = 'data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,'
            , template = '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40"><head><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>{Worksheet}</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--><meta http-equiv="content-type" content="xlsx"/></head><body><table>{table}</table></body></html>'
        ...
        for (var j = 0; j < cells.length; j++) {
          ...
          const xlfile = document.createElement('script');
          xlfile.src = 'https://cdn.sheetjs.com/xlsx-0.19.2/package/dist/xlsx.full.min.js';
          document.head.appendChild(xlfile);
